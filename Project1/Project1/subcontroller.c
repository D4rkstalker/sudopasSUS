...
void tutorialMovement(int isplayer) {
	...
	if (CP_Input_MouseTriggered(MOUSE_BUTTON_1)) {
			...
			CP_Vector outv = CP_Vector_Normalize(CP_Vector_Subtract(CP_Vector_Set((float)(CP_Input_GetMouseWorldX() - WorldX), (float)(CP_Input_GetMouseWorldY() - WorldY)), CP_Vector_Set((float)(x - WorldX), (float)(y - WorldY))));
			float a = CP_Math_Degrees((float)atan2(outv.y, outv.x));
			for (int i = -18; i < 18; i++) {
				CP_Vector v = AngleToVector((float)(a + i * 1));
...
void settings_menu(void) {
	...
	CP_Font_DrawText("Pause Menu", CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f - 200.f);

	CP_Settings_TextSize(40.0f);
	CP_Settings_Fill(CP_Color_Create(220, 220, 220, 255));
	CP_Font_DrawText("Resume Game", CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f - 100.f);
	//CP_Font_DrawText("Retry from Last Check Point", CP_System_GetWindowWidth() / 2, CP_System_GetWindowHeight() / 2 );
	CP_Font_DrawText("Retry from Last Check Point", CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f + 100.f);
	CP_Font_DrawText("Retry Game", CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f + 200.f);
	CP_Font_DrawText("Return to Menu", CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f + 300.f);

	CP_Settings_Stroke(CP_Color_Create(220, 220, 220, 255));
	CP_Settings_RectMode(CP_POSITION_CENTER);
	CP_Settings_Fill(CP_Color_Create(220, 220, 220, settings_alpha_1));
	CP_Graphics_DrawRect(CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f - 110.f, 500.f, 50.f);
	CP_Settings_Fill(CP_Color_Create(220, 220, 220, settings_alpha_2));
	CP_Graphics_DrawRect(CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f - 10.f, 500.f, 50.f);
	CP_Settings_Fill(CP_Color_Create(220, 220, 220, settings_alpha_3));
	CP_Graphics_DrawRect(CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f + 90.f, 500.f, 50.f);
	CP_Settings_Fill(CP_Color_Create(220, 220, 220, settings_alpha_4));
	CP_Graphics_DrawRect(CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f + 190.f, 500.f, 50.f);
	CP_Settings_Fill(CP_Color_Create(220, 220, 220, settings_alpha_5));
	CP_Graphics_DrawRect(CP_System_GetWindowWidth() / 2.f, CP_System_GetWindowHeight() / 2.f + 290.f, 500.f, 50.f);


	if (CP_Input_GetMouseWorldX() >= CP_System_GetWindowWidth() / 2.f - 250.f && CP_Input_GetMouseWorldX() <= CP_System_GetWindowWidth() / 2.f + 250.f && CP_Input_GetMouseWorldY() >= CP_System_GetWindowHeight() / 2.f - 145.f && CP_Input_GetMouseWorldY() <= CP_System_GetWindowHeight() / 2.f - 85.f) {
		settings_alpha_1 = 50;
		if (CP_Input_MouseTriggered(MOUSE_BUTTON_1)) {
			game_states = resume;
		}
...
void volumeControl(void) {
	...
	float screenborderX = CP_System_GetWindowWidth() / 4.f;
	float screenborderY = CP_System_GetWindowHeight() / 4.f;
	float screensizeX = CP_System_GetWindowWidth() / 2.f;
	float screensizeY = CP_System_GetWindowHeight() / 2.f;

	CP_Vector volumeMin = CP_Vector_Set(screenborderX + screensizeX / 4.f, (screenborderY + screensizeY / 2.f) - 10.f);

	CP_Vector volumeMax = CP_Vector_Set(screenborderX + screensizeX - (screensizeX / 4.f), (screenborderY + (screensizeY / 2.f) - 10.f)); //screensizeY - 
...
void movement(void) {
	...
	if (game_states == resume) {
		...
		if (CP_Input_KeyDown(KEY_W) && CP_Input_KeyDown(KEY_S)) {
			...
			if (CP_Input_KeyDown(KEY_W)) {
				...
				player1.velocity_y -= player1.acceleration_y * 0.1f;
				time++;
		...
		if (CP_Input_KeyDown(KEY_D) && CP_Input_KeyDown(KEY_A)) {
			...
			if (CP_Input_KeyDown(KEY_D)) {
				...
				player1.velocity_x += player1.acceleration_x * 0.1f;
				time++;
			}
		...
		player1.velocity_y *= 0.9f;
		player1.velocity_x *= 0.9f;
	...
	game_states == theMap ?
		...
		CP_Graphics_DrawRect(CP_System_GetWindowWidth() / 4.f, CP_System_GetWindowHeight() / 4.f, (CP_System_GetWindowWidth() + 50.f) / 2.f, (CP_System_GetWindowHeight() + 50.f) / 2.f),
		mapScale() : 0;
}
