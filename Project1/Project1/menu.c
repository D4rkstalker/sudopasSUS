...
void menu_button(int button, int* buttonalpha, float y) {
	if (CP_Input_GetMouseWorldX() >= (float)((float)CP_System_GetWindowWidth() / 2 - y) && CP_Input_GetMouseWorldX() <= (float)((float)CP_System_GetWindowWidth() / 2 + 350) && CP_Input_GetMouseWorldY() >= (float)(y - 50) && CP_Input_GetMouseWorldY() <= (float)(y + 50)) {
		*buttonalpha = 50;
...
void draw_button(char* str, int button, float y) {
	...
	CP_Graphics_DrawRect((float)(CP_System_GetWindowWidth() / 2), y, 700, 100);
	CP_Settings_Fill(CP_Color_Create(255, 255, 255, 255));
	CP_Settings_TextSize(100);
	CP_Font_DrawText(str, (float)(CP_System_GetWindowWidth() / 2), y);
}
...
void mainmenu_update(void) {
	...
	CP_Font_DrawText("Project SONAR", (float)(CP_System_GetWindowWidth() / 2), 200);
